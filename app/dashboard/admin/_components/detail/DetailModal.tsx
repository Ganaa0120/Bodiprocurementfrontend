        ...
        >
          ...
          >
            <ClickableAvatar
              ...
              name={person.first_name ?? person.email}
            />
          ...
          >
            ...
            {person.supply_direction && (
              <Section icon={Briefcase} title="Үйл ажиллагаа">
                ...
                <Row
                  label="Эхэлсэн огноо"
                  value={fmtDate(person.activity_start_date)}
                />
                <Row
                  label="НӨАТ"
                  value={person.is_vat_payer ? "Тийм" : "Үгүй"}
                />
                ...
                {person.activity_directions?.length > 0 && (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {person.activity_directions.map(
                        (d: any, index: number) => (
                          <span
                            key={index} // эсвэл key={String(d)}
                            style={{
                              fontSize: 10,
                              padding: "2px 8px",
                              borderRadius: 99,
            ...
            {docs.length > 0 && (
              <div>
                ...
                >
                  {docs.map((d) => (
                    <DocThumbnail key={d.label} url={d.url} label={d.label} />
                  ))}
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {ACTIONS.filter((a) => a.status !== person.status).map((a) => (
                  <button
                    key={a.status}
                    onClick={() => onUpdateStatus(person.id, a.status)}
                    disabled={loading}
                    style={{
                      padding: "9px 18px",
                      borderRadius: 11,
