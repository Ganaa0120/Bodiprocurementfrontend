...
import {
  X, FileText, Loader2, Trash2, Building2, User,
  CreditCard, MapPin, Briefcase, CheckCircle2, AlertCircle, Download, Eye,
} from "lucide-react";
...
function DocViewerModal({ url, label, onClose }: {
  ...
  const content = (
    <div style={{ position:"fixed",inset:0,zIndex:99999,background:"rgba(0,0,0,0.96)",
        ...
        <div style={{ display:"flex",gap:8 }}>
          <a href={url} target="_blank" rel="noopener noreferrer"
            ...
            style={{ display:"flex",alignItems:"center",gap:6,padding:"7px 13px",
              borderRadius:9,background:"rgba(59,130,246,0.1)",
          ...
          <button onClick={onClose}
            style={{ background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",
              borderRadius:9,padding:"7px 10px",cursor:"pointer",color:"rgba(255,255,255,0.5)",display:"flex" }}>
            <X size={15}/>
          </button>
        </div>
      </div>
      ...
      <div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",
        ...
        {isImg ? (
          ...
          <object data={`${url}#toolbar=1&navpanes=0`} type="application/pdf"
          ...
          <div style={{ textAlign:"center",color:"rgba(255,255,255,0.5)",padding:32 }}>
            <FileText size={48} style={{ margin:"0 auto 16px",opacity:0.3,display:"block" }}/>
            <p style={{ fontSize:14,marginBottom:20 }}>Энэ файлын төрлийг харуулах боломжгүй</p>
            <a href={url} target="_blank" rel="noopener noreferrer"
              style={{ display:"inline-flex",alignItems:"center",gap:8,padding:"11px 22px",
                borderRadius:12,background:"rgba(59,130,246,0.15)",
                border:"1px solid rgba(59,130,246,0.3)",color:"#60a5fa",
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ width:110,flexShrink:0,cursor:"pointer",borderRadius:12,overflow:"hidden",
          ...
          {isImg ? (
            ...
            <div style={{ textAlign:"center" }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none"
                stroke="rgba(248,113,113,0.65)" strokeWidth={1.5}>
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                <polyline points="14 2 14 8 20 8"/>
                <line x1="16" y1="13" x2="8" y2="13"/>
                <line x1="16" y1="17" x2="8" y2="17"/>
              </svg>
              <div style={{ fontSize:9,color:"#f87171",marginTop:4,fontWeight:700,letterSpacing:"0.05em" }}>PDF</div>
            </div>
          )}
          {/* Hover overlay */}
          <div style={{ position:"absolute",inset:0,background:"rgba(59,130,246,0.2)",
        ...
        {/* Label */}
        <div style={{ padding:"5px 7px",background:"rgba(0,0,0,0.35)",
...
// ── DetailModal ───────────────────────────────────────────────
export function DetailModal({ org, onClose, onStatusChange, onDeleted, showToast,
  canEditStatus = true, canDelete = true }: {
    ...
    <>
      ...
      <div style={{ position:"fixed",inset:0,zIndex:9000,overflowY:"auto",
        ...
        <div style={{ minHeight:"100%",display:"flex",alignItems:"center",
          ...
          <div style={{ width:"100%",maxWidth:660,background:"#0d1526",
            border:"1px solid rgba(255,255,255,0.08)",borderRadius:22,
            boxShadow:"0 32px 80px rgba(0,0,0,0.7)",animation:"modalIn .25s ease" }}
            onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding:"22px 26px",borderBottom:"1px solid rgba(255,255,255,0.06)",
              ...
              <div style={{ flex:1,minWidth:0 }}>
                ...
                {org.supplier_number && (
                  <div style={{ fontSize:10,fontFamily:"monospace",
                    color:"rgba(148,163,184,0.35)",marginTop:1 }}>
                    {org.supplier_number}
                  </div>
                )}
              ...
              <button onClick={onClose}
                style={{ background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.07)",
                  borderRadius:9,padding:8,cursor:"pointer",color:"rgba(148,163,184,0.5)",
            ...
            <div style={{ padding:"22px 26px",display:"flex",flexDirection:"column",gap:18 }}>
              ...
              <Sec icon={Building2} label="Байгууллагын мэдээлэл">
                ...
                <Row label="Улсын бүртгэл №"    value={org.state_registry_number}/>
                <Row label="Байгууллагын төрөл" value={org.company_type}/>
                <Row label="И-мэйл"             value={org.email}/>
                <Row label="Утас"               value={org.phone}/>
                <Row label="Байгуулагдсан"      value={fmtDate(org.established_date)}/>
                ...
                <Row label="Тусгай зөвшөөрөл"   value={org.has_special_permission}/>
                {org.has_special_permission && <>
                  <Row label="Зөвшөөрлийн №"  value={org.special_permission_number}/>
                  <Row label="Дуусах хугацаа" value={fmtDate(org.special_permission_expiry)}/>
                </>}
                ...
                <Sec icon={User} label="Эзэмшигчийн мэдээлэл">
                  {org.beneficial_owners.map((o: any, i: number) => (
                    <div key={i} style={{
                      ...
                      <Row label="Овог / Нэр"    value={`${o.last_name||""} ${o.first_name||""}`.trim()}/>
              ...
              {!org.beneficial_owners?.length && (org.owner_first_name||org.owner_last_name) && (
              ...
              {/* Үйл ажиллагаа */}
              {(org.supply_direction || org.activity_description || org.activity_directions?.length > 0) && (
                <Sec icon={Briefcase} label="Үйл ажиллагаа">
                  <Row label="Нийлүүлэх чиглэл" value={supply(org.supply_direction)}/>
                  <Row label="Тайлбар"           value={org.activity_description}/>
                  {org.activity_directions?.length > 0 && (
                    <div style={{ padding:"7px 0" }}>
                      <div style={{ display:"flex",flexWrap:"wrap",gap:5 }}>
                        {org.activity_directions.map((d: string) => (
                          <span key={d} style={{ fontSize:10,padding:"2px 8px",borderRadius:99,
                            background:"rgba(59,130,246,0.12)",color:"#60a5fa",
                            border:"1px solid rgba(59,130,246,0.2)" }}>{d}</span>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {docs.length > 0 && (
                <div>
                  <div style={{ display:"flex",alignItems:"center",gap:7,marginBottom:10 }}>
                    ...
                    <span style={{ fontSize:10,color:"rgba(148,163,184,0.25)" }}>
                  ...
                  <div className="doc-scroll"
                    ...
                    onClick={e => e.stopPropagation()}>
                    {docs.map(d => <DocCard key={d.label} url={d.url} label={d.label}/>)}
                  </div>
                </div>
              )}
              ...
              {org.return_reason && (
                <div style={{ padding:"10px 14px",borderRadius:12,
                  ...
                  <div style={{ fontSize:12,color:"rgba(255,255,255,0.6)" }}>{org.return_reason}</div>
                </div>
              )}
              ...
              <div>
                ...
                <textarea value={returnReason} onChange={e => setReturnReason(e.target.value)}
                  rows={2} placeholder="Жишээ: Баримт дутуу байна..."
                  ...
                  {canEditStatus && STATUS_ACTIONS.filter(a => a.status !== localStatus).map(a => (
                    <button key={a.status} onClick={() => doAction(a.status)} disabled={actLoad}
                      style={{ padding:"9px 18px",borderRadius:10,background:a.bg,
                        ...
                        gap:6,opacity:actLoad?0.5:1,transition:"all .15s" }}>
                      {actLoad
                        ? <Loader2 size={13} style={{ animation:"spin 0.8s linear infinite" }}/>
                        : a.status==="approved" ? <CheckCircle2 size={13}/>
                        : a.status==="returned" ? <AlertCircle size={13}/> : null}
                      {a.label}
                    </button>
                  ))}
                </div>
                    ...
                    <div style={{ display:"flex",alignItems:"center",gap:10,padding:"12px 14px",
                      ...
                      <button onClick={() => setConfirmDel(false)}
                        style={{ padding:"5px 12px",borderRadius:8,
                          ...
                          color:"rgba(148,163,184,0.6)",fontSize:12,cursor:"pointer",fontFamily:"inherit" }}>
                        Болих
                      </button>
