...
type Direction = { id: number; label: string; is_active: boolean; created_at?: string };

// ── Modal ─────────────────────────────────────────────────────
function DirectionModal({ mode, direction, onClose, onSave, showToast }: {
  mode:"create"|"edit"; direction?: Direction|null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast(mode === "create" ? "Чиглэл нэмэгдлэа ✓" : "Амжилттай хадгаллаа ✓");
      onSave();
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
          <div>
            ...
            <input value={label} onChange={e=>setLabel(e.target.value)}
              style={inp} placeholder="Жишээ: Барилга угсралт"
...
// ── Directions Tab ────────────────────────────────────────────
export function DirectionsTab({ isSuperAdmin, showToast }: {
  ...
  const fetchDirections = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) setDirections(data.directions ?? []);
    } catch {}
  ...
  const handleToggle = async (d: Direction) => {
    ...
    try {
      ...
      setDirections(prev => prev.map(x => x.id===d.id ? {...x, is_active:!x.is_active} : x));
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`${API}/api/activity-directions/${confirmDel.id}`, {
        method:"DELETE", headers:{ Authorization:`Bearer ${getToken()}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message ?? "Алдаа гарлаа");
      showToast(`${confirmDel.label} устгагдлаа`);
      setDirections(prev => prev.filter(x => x.id !== confirmDel.id));
      setConfirmDel(null);
    } catch(e:any) { showToast(e.message, false); }
    finally { setDeleting(false); }
  };

  const active   = directions.filter(d => d.is_active).length;
  const inactive = directions.filter(d => !d.is_active).length;
  ...
  return (
    <>
      ...
      {(modalMode==="create"||modalMode==="edit") && (
        <DirectionModal mode={modalMode} direction={editTarget}
          onClose={()=>{ setModalMode(null); setEditTarget(null); }}
          onSave={()=>{ setModalMode(null); setEditTarget(null); fetchDirections(); }}
          showToast={showToast}/>
      )}
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div style={{ display:"flex",gap:12,alignItems:"center" }}>
            <span style={{ fontSize:12,color:"rgba(148,163,184,0.4)" }}>
              {loading?"...":directions.length+" чиглэл"}
            </span>
            {!loading && (
              <>
                ...
                {directions.map((d, i) => (
                  <tr key={d.id} className="tr">
                    <td style={{ padding:"12px 16px",fontSize:11,
                      color:"rgba(148,163,184,0.3)",fontFamily:"monospace",width:48 }}>
                      {i + 1}
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                        <div style={{ width:32,height:32,borderRadius:9,flexShrink:0,
                          background: d.is_active?"rgba(59,130,246,0.12)":"rgba(148,163,184,0.06)",
                          border: d.is_active?"1px solid rgba(59,130,246,0.2)":"1px solid rgba(148,163,184,0.1)",
                          display:"flex",alignItems:"center",justifyContent:"center",
                          fontSize:12,fontWeight:700,
                          color: d.is_active?"#60a5fa":"rgba(148,163,184,0.4)" }}>
                          {d.label[0]}
                        </div>
                        <span style={{ fontSize:13,fontWeight:600,
                          color: d.is_active?"rgba(255,255,255,0.85)":"rgba(255,255,255,0.35)" }}>
                          {d.label}
                        </span>
                      </div>
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      {/* Статус — дарж toggle хийнэ */}
                      <button onClick={()=>isSuperAdmin&&handleToggle(d)}
                        ...
                        style={{ display:"inline-flex",alignItems:"center",gap:5,
                          ...
                          background: d.is_active?"rgba(16,185,129,0.12)":"rgba(148,163,184,0.1)",
                    ...
                    <td style={{ padding:"12px 16px",fontSize:11,color:"rgba(148,163,184,0.4)" }}>
                      {d.created_at?new Date(d.created_at).toLocaleDateString("mn-MN"):"—"}
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                      {isSuperAdmin && (
                        <div style={{ display:"flex",gap:5 }}>
                          <button onClick={()=>{ setEditTarget(d); setModalMode("edit"); }}
