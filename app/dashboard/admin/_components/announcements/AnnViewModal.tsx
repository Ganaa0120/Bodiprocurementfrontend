    ...
    >
      ...
      <div
        style={{
          fontSize: 13,
          fontWeight: 500,
          color: warn ? "#f87171" : "rgba(255,255,255,0.82)",
        }}
      >
        {value}
      </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {ann.category_name && (
              <InfoItem label="Категори" value={`📁 ${ann.category_name}`} />
            )}
            {ann.deadline && (
              <InfoItem
                label="Дуусах хугацаа"
                value={new Date(ann.deadline).toLocaleDateString("mn-MN")}
                warn={new Date(ann.deadline) < new Date()}
              />
            )}
            {(ann.budget_from || ann.budget_to) && (
              <InfoItem
                ...
                value={
                  [
                    ann.budget_from && ann.budget_from.toLocaleString(),
                    ann.budget_to && ann.budget_to.toLocaleString(),
                  ]
                    .filter(Boolean)
                    .join(" — ") + ` ${ann.currency ?? "MNT"}`
                }
              />
            )}
            {ann.view_count !== undefined && (
              <InfoItem label="Үзсэн" value={`${ann.view_count} удаа`} />
            )}
          </div>
