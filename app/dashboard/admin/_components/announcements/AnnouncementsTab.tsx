...
function ConfirmDel({
  ...
  const go = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/announcements/${ann.id}`, {
        method: "DELETE",
        headers: authH(),
      });
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <p
            style={{ fontSize: 13, color: "rgba(148,163,184,0.5)", margin: 0 }}
          >
...
export function AnnouncementsTab({
  ...
  // ЭНД БАЙСАН ХЭСГИЙГ ОДООХУНД ИЙМ БОЛГОЖ ЗАСААРАЙ:
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(error); // Алдаа гарвал консол дээр хэвлэнэ
    } finally {
      setLoading(false);
    }
  }, [statusF, typeF]);
  ...
  const updateBidStatus = async (bidId: string, status: string) => {
    ...
    try {
      const res = await fetch(
        `${API}/api/announcements/${bidsAnn.id}/bids/${bidId}/status`,
        {
          method: "PATCH",
          headers: jsonH(),
  ...
  const fmt = (v?: number, c = "MNT") =>
    v ? `${v.toLocaleString()} ${c}` : null;
    ...
    <>
      ...
      {delAnn && (
        <ConfirmDel
          ...
          onDone={() => {
            setAnns((p) => p.filter((a) => a.id !== delAnn!.id));
            setDelAnn(null);
      ...
      {viewAnn && (
        <AnnViewModal ann={viewAnn} onClose={() => setViewAnn(null)} />
      )}
      {bidsAnn && (
        <BidsDrawer
          ann={bidsAnn}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding: "20px 24px",
                    border: "1px solid #334155",
                    borderLeft: `3px solid ${tc?.color ?? "#4f46e5"}`,
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        {ann.ann_type === "rfq" && ann.rfq_quantity && (
                          ...
                          >
                            <Layers size={11} /> {ann.rfq_quantity}{" "}
                            {ann.rfq_unit}
                          </span>
                        )}
                        <span
                      ...
                      {ann.description && (
                        ...
                        >
                          {stripHtml(ann.description)}
                          {ann.description.length > 120 ? "…" : ""}
                        </p>
                      )}

                      {/* Activity directions — string эсвэл {main_id, sub_ids} object аль алинд тохирно */}
                          ...
                          <>
                            <div
                              style={{
                                ...
                                fontSize: 10,
                                fontWeight: 700,
                                color: ann.is_mine ? "#a5b4fc" : "#fbbf24",
                              }}
                            >
