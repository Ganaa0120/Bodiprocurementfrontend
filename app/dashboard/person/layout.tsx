...
import {
  LayoutDashboard, FileText, Bell, User, LogOut, ChevronRight,
  Menu, X, Building2, ClipboardList, Megaphone,
} from "lucide-react";
...
export default function PersonLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // ✅ Parse хийж role шалгана
    if (raw) {
      try {
        const parsed = JSON.parse(raw);
        // individual эсвэл role байхгүй бол зөвшөөрнө
    ...
    // ✅ Бүрэн мэдээллийг татна
    const refreshUser = () => {
      const t = localStorage.getItem("token");
      if (!t) return;
      fetch(`${API}/api/persons/me`, { headers: { Authorization: `Bearer ${t}` } })
        .then(r => r.json())
        .then(d => {
    ...
    // ✅ Мэдэгдэл — зөв endpoint
    const fetchNotifications = () => {
      const t = localStorage.getItem("token");
      if (!t) return;
      fetch(`${API}/api/notifications/mine`, { headers: { Authorization: `Bearer ${t}` } })
        .then(r => r.json())
        .then(d => {
    ...
    const interval = setInterval(refreshUser, 30 * 60 * 1000); // 30 мин
  ...
  const pStatus   = STATUS_CFG[user?.status as keyof typeof STATUS_CFG] ?? STATUS_CFG.pending;
  const isReturned = user?.status === "returned";
  const initials  = [user?.last_name?.[0], user?.first_name?.[0]].filter(Boolean).join("").toUpperCase() || "?";
  ...
  const currentPage = NAV.find(n =>
    pathname === n.href || (n.href !== "/dashboard/person" && pathname.startsWith(n.href))
  );
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", background:"#f8f9fc", fontFamily:"'Inter',sans-serif" }}>
      ...
      <aside className={`sidebar-wrap ${open ? "open" : ""}`}>
        ...
        <div style={{ padding:"14px 20px", borderBottom:"1px solid #f8fafc" }}>
          ...
          <span style={{ display:"inline-flex", alignItems:"center", gap:5,
            ...
            <span style={{ width:5, height:5, borderRadius:"50%", background:pStatus.dot,
              animation:user?.status==="pending"?"pulse 1.5s infinite":"none" }}/>
            {pStatus.label}
          </span>
        ...
        <nav style={{ flex:1, padding:"12px", overflowY:"auto" }}>
          <p style={{ fontSize:10, fontWeight:600, color:"#cbd5e1", letterSpacing:"0.1em",
            textTransform:"uppercase" as const, padding:"0 8px", marginBottom:6 }}>
            Үндсэн
          </p>
              ...
              <Link key={href} href={href} onClick={() => setOpen(false)}
                ...
                {badge > 0
                  ? <span style={{ fontSize:10, fontWeight:800, padding:"1px 7px",
                      borderRadius:99, background:active?"rgba(255,255,255,0.3)":"#ef4444",
        ...
        <div style={{ padding:"12px", borderTop:"1px solid #f8fafc" }}>
          <button onClick={logout}
            style={{ width:"100%", display:"flex", alignItems:"center", gap:10,
              ...
              cursor:"pointer", fontSize:13, color:"#94a3b8", transition:"all .15s",
              fontFamily:"inherit" }}
      ...
      <div className="main-wrap">
        {/* Topbar */}
        <header style={{ height:56, background:"white", borderBottom:"1px solid #f1f5f9",
          padding:"0 24px", display:"flex", alignItems:"center", gap:12,
          position:"sticky", top:0, zIndex:20, boxShadow:"0 1px 8px rgba(0,0,0,0.04)" }}>
          {/* Mobile menu button */}
          <button className="mob-btn" onClick={() => setOpen(!open)}
            style={{ padding:6, borderRadius:8, border:"none", background:"none", cursor:"pointer", alignItems:"center" }}>
            {open ? <X size={18} style={{ color:"#64748b" }}/> : <Menu size={18} style={{ color:"#64748b" }}/>}
          </button>
          ...
          {/* Bell */}
          <Link href="/dashboard/person/notifications"
            style={{ padding:"7px", borderRadius:9, border:"1px solid #f1f5f9",
              background:"#fafafa", display:"flex", textDecoration:"none", position:"relative" }}>
