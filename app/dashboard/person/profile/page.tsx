...
import {
  ...
  User, MapPin, Briefcase, FileText, Bell, CheckCircle, Camera, Shield,
} from "lucide-react";
...
function calcPct(form:any, selDirs:number[], previews:Record<string,string>) {
  const scored = [
    ...
    <div>
      ...
      {editing ? (
        ...
        <div style={{ fontSize:13, color:display?"#1e293b":"#cbd5e1", padding:"10px 0",
          ...
          fontFamily:mono?"monospace":"inherit", wordBreak:"break-word", overflowWrap:"break-word" }}>
          {display||"—"}
        </div>
      )}
    </div>
        ...
        <div style={{ position:"relative" }}>
          <select value={value} onChange={e=>onChange(e.target.value)}
            style={{ width:"100%", padding:"10px 36px 10px 14px", borderRadius:10,
              ...
              color:value?"#1e293b":"#94a3b8", outline:"none", background:"white",
        ...
        {options.map((o:any) => {
          ...
          return editing ? (
            ...
            <span key={String(o.value)}
              style={{ fontSize:13, padding:"9px 0", color:sel?"#4f46e5":"#cbd5e1",
                fontWeight:sel?600:400, borderBottom:sel?"2px solid #6366f1":"1px solid #f1f5f9",
      ...
      <div onClick={()=>editing&&inputRef.current?.click()}
        ...
        {preview ? (
          ...
          <div style={{ textAlign:"center", padding:16 }}>
            ...
            {editing&&isDocField&&<p style={{ fontSize:10, color:"#cbd5e1", margin:"3px 0 0" }}>PDF, Word, Excel, зураг</p>}
          </div>
        )}
        {editing&&preview&&(
...
export default function PersonProfilePage() {
    ...
    fetch(`${API}/api/persons/me`,{ headers:{ Authorization:`Bearer ${token}` } })
      .then(r=>r.json()).then(d=>{
        if(d.success&&(d.person||d.user)){
          ...
          setProfile(u);
          const f={
            family_name:u.family_name||"", last_name:u.last_name||"", first_name:u.first_name||"",
            birth_date:u.birth_date?.slice(0,10)||"", gender:u.gender||"", phone:u.phone||"",
            aimag_niislel:u.aimag_niislel||"", sum_duureg:u.sum_duureg||"",
            bag_horoo:u.bag_horoo||"", toot:u.toot||"",
            address_different:u.address_different||false, orshisuugaa_hayag:u.orshisuugaa_hayag||"",
            activity_description:u.activity_description||"",
            activity_start_date:u.activity_start_date?.slice(0,10)||"",
            is_vat_payer:u.is_vat_payer||false, notification_type:u.notification_type||"email",
          };
  ...
  const startEdit  = () => { setSnapshot({...form}); setSelDirSnap([...selDirs]); setEditing(true); setError(""); };
  const cancelEdit = () => { setForm({...snapshot}); setSelDirs([...selDirSnap]); setEditing(false); setError(""); };

  const handleSave = async () => {
    const errs = validateMongolianForm(form, [
      "family_name","last_name","first_name","sum_duureg",
  ...
  return (
    <div style={{ maxWidth:860, margin:"0 auto",
        ...
        <div style={{ position:"sticky", top:0, zIndex:50, animation:"fadeIn .2s ease",
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <div>
              ...
              {isNewUser&&!isMobile&&(
                <span style={{ fontSize:11, color:"#94a3b8", marginLeft:8 }}>· Доорх талбаруудыг бөглөөд хадгална уу</span>
              )}
            </div>
          </div>
      ...
      <Card>
        <div style={{ display:"flex", flexDirection: isMobile?"column":"row" as any,
          ...
          <div style={{ flexShrink:0, display:"flex",
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              {!isMobile&&(
                <div style={{ textAlign:"right" as const }}>
                  <div style={{ fontSize:11, color:"#94a3b8", marginBottom:2 }}>Анкет бөглөлт</div>
                  {pct<100&&<div style={{ fontSize:10, color:"#f59e0b" }}>Бүрэн биш</div>}
                </div>
              )}
              <Ring pct={pct}/>
            </div>
            ...
            {editing&&!isNewUser&&(
              <div style={{ fontSize:11, color:"#f59e0b", fontWeight:600, padding:"5px 10px",
        ...
        {pct<100&&!editing&&(
          <div style={{ marginTop:16, padding:"12px 14px", borderRadius:10,
            background:"#fafafa", border:"1px solid #f1f5f9", display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ fontSize:18 }}>💡</div>
            <div>
              <div style={{ fontSize:12, fontWeight:600, color:"#0f172a", marginBottom:2 }}>Анкетаа бүрэн бөглөнө үү</div>
              <div style={{ fontSize:11, color:"#64748b" }}>
                {!form.gender&&"Хүйс · "}{!form.birth_date&&"Төрсөн огноо · "}
                {selDirs.length===0&&"Нийлүүлэх чиглэл · "}{!previews.profile_photo&&"Профайл зураг · "}
        ...
        {profile?.status==="returned"&&(
          <div style={{ marginTop:16, borderRadius:14, border:"1.5px solid #fecaca", overflow:"hidden" }}>
            <div style={{ padding:"12px 16px", background:"linear-gradient(135deg,#fef2f2,#fff5f5)",
      ...
      <Card>
        ...
        <div style={{ display:"grid", gridTemplateColumns:g3, gap:16, marginBottom:16 }}>
          <FInput label="Ургийн овог *" value={form.family_name} editing={editing}
            onChange={(v:string)=>{ F("family_name",v); setFieldErrors(p=>({...p,family_name:v&&!isMongolian(v)?"Ургийн овог криллээр бичнэ үү":""})); }}
            fieldError={fieldErrors.family_name}/>
          <FInput label="Овог *" value={form.last_name} editing={editing}
            onChange={(v:string)=>{ F("last_name",v); setFieldErrors(p=>({...p,last_name:v&&!isMongolian(v)?"Овог криллээр бичнэ үү":""})); }}
            fieldError={fieldErrors.last_name}/>
          <FInput label="Нэр *" value={form.first_name} editing={editing}
            onChange={(v:string)=>{ F("first_name",v); setFieldErrors(p=>({...p,first_name:v&&!isMongolian(v)?"Нэр криллээр бичнэ үү":""})); }}
        ...
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:16, marginBottom:16 }}>
          ...
          <FRadio label="Хүйс *" value={form.gender} onChange={(v:string)=>F("gender",v)}
            editing={editing} options={[{value:"male",label:"Эрэгтэй"},{value:"female",label:"Эмэгтэй"}]}/>
        </div>
      ...
      <Card>
        ...
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:16, marginBottom:16 }}>
          ...
          <FInput label="Байр, тоот" value={form.toot} editing={editing}
            onChange={(v:string)=>F("toot",v)} placeholder="Байр болон тоот оруулах"/>
        </div>
        <div style={{ marginBottom:12 }}>
          <FRadio label="Оршин суугаа хаяг бүртгэлийнхээс өөр үү?" value={form.address_different}
      ...
      {/* ── 3. Үйл ажиллагаа ── */}
      <Card>
        <SectionTitle icon={Briefcase} title="Үйл ажиллагаа" subtitle="Үйл ажиллагаа явуулж буй чиглэл"/>
        <div style={{ marginBottom:16 }}>
          <label style={S.label}>Нийлүүлэх чиглэл *</label>
          <div style={{ display:"flex", flexWrap:"wrap" as const, gap:7 }}>
            {dirs.length===0
              ? <span style={{ fontSize:12, color:"#94a3b8" }}>Чиглэл ачаалж байна...</span>
              : dirs.map(d=>{
                  const on=selDirs.includes(d.id);
                  return (
                    <button key={d.id} type="button" onClick={()=>editing&&toggleDir(d.id)}
                      style={{ padding:"7px 16px", borderRadius:99, fontSize:12, fontFamily:"inherit",
                        border:on?"1.5px solid #6366f1":"1.5px solid #e2e8f0",
                        background:on?"#eef2ff":"white", color:on?"#4f46e5":"#64748b",
                        cursor:editing?"pointer":"default", fontWeight:on?600:400, transition:"all .12s" }}>
                      {d.label}
                    </button>
                  );
                })}
        ...
        <div style={{ marginBottom:16 }}>
          <FInput label="Үйл ажиллагаа эхэлсэн огноо *" type="date" value={form.activity_start_date}
            onChange={(v:string)=>F("activity_start_date",v)} editing={editing}/>
        </div>
        <div>
      ...
      <Card>
        ...
        <div style={{ display:"grid", gridTemplateColumns:gDoc, gap:12 }}>
          <DocCard label="Профайл зураг" fieldKey="profile_photo"
            preview={previews.profile_photo} onFile={onFile} editing={editing} accept="image/*" required/>
          <DocCard label="Иргэний үнэмлэх (урд)" fieldKey="id_card_front"
            preview={previews.id_card_front} onFile={onFile} editing={editing} accept="image/*" required/>
          <DocCard label="Иргэний үнэмлэх (ард)" fieldKey="id_card_back"
            preview={previews.id_card_back} onFile={onFile} editing={editing} accept="image/*" required/>
          <DocCard label="Үйл ажиллагааны танилцуулга" fieldKey="activity_intro"
            preview={previews.activity_intro} onFile={onFile} editing={editing}
      ...
      <Card>
        <SectionTitle icon={Bell} title="Мэдэгдлийн тохиргоо" subtitle="Зар болон системийн мэдэгдлийг хэрхэн хүлээн авах"/>
        <FSelect label="Мэдэгдэл хүлээн авах арга" value={form.notification_type}
          onChange={(v:string)=>F("notification_type",v)} editing={editing}
          options={[{value:"email",label:"И-мэйлээр"}]}/>
      </Card>
    </div>
  );
}