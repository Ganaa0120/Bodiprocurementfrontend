...
import {
  Pencil, X, Check, Upload, Loader2, AlertCircle,
  ChevronDown, User, MapPin, Briefcase, FileText,
  Bell, CheckCircle, Camera, Shield,
} from "lucide-react";
...
function FInput({ label, value, onChange, editing, type = "text", placeholder, mono = false }: any) {
  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <div style={{ fontSize:13, color: display ? "#1e293b" : "#cbd5e1",
          padding:"10px 0", borderBottom:"1px solid #f1f5f9", fontWeight: display ? 500 : 400,
  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <div style={{ fontSize:13, color: value ? "#1e293b" : "#cbd5e1",
          ...
          return editing ? (
            <button key={String(o.value)} type="button" onClick={() => onChange(o.value)}
              style={{ flex:1, padding:"9px 12px", borderRadius:10, cursor:"pointer",
                ...
                transition:"all .15s", fontFamily:"inherit" }}>{o.label}</button>
          ) : (
            <span key={String(o.value)} style={{ fontSize:13, padding:"9px 0",
              ...
              flex:1, textAlign:"center" as const }}>{o.label}</span>
          );
        })}
      </div>
    </div>
...
export default function PersonProfilePage() {
  ...
  return (
    <div style={{ maxWidth:860, margin:"0 auto", padding:"20px 16px 40px",
      ...
      {saved && (
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px",
          borderRadius:12, background:"#f0fdf4", border:"1px solid #a7f3d0",
          animation:"fadeIn .3s ease" }}>
      ...
      {/* ── Hero ──────────────────────────────────────────────── */}
      <Card>
        <div className="profile-hero" style={{ display:"flex", alignItems:"center", gap:20 }}>
          ...
          <label style={{ cursor: editing ? "pointer" : "default", flexShrink:0, position:"relative" }}>
            <div style={{ width:76, height:76, borderRadius:20, overflow:"hidden",
              ...
              {previews.profile_photo
                ? <img src={previews.profile_photo} alt=""
          ...
          {/* Progress + button */}
          <div style={{ flexShrink:0, display:"flex", flexDirection:"column" as const,
      ...
      <Card>
        ...
        <div className="profile-grid3"
          ...
          <FInput label="Ургийн овог" value={form.family_name}
            onChange={(v:string) => F("family_name",v)} editing={editing}/>
          <FInput label="Овог" value={form.last_name}
            onChange={(v:string) => F("last_name",v)} editing={editing}/>
          <FInput label="Нэр" value={form.first_name}
            onChange={(v:string) => F("first_name",v)} editing={editing}/>
        </div>
        <div className="profile-grid2"
          ...
          <FRadio label="Хүйс" value={form.gender}
            onChange={(v:string) => F("gender",v)} editing={editing}
      ...
      <Card>
        ...
        <div className="profile-grid2"
          style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
          <FSelect label="Аймаг / Нийслэл" value={form.aimag_niislel}
            onChange={(v:string) => F("aimag_niislel",v)}
            options={AIMAG} placeholder="Сонгох" editing={editing}/>
          <FInput label="Сум / Дүүрэг" value={form.sum_duureg}
            onChange={(v:string) => F("sum_duureg",v)} editing={editing}/>
        </div>
        <div className="profile-grid2"
          style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
          <FInput label="Баг / Хороо" value={form.bag_horoo}
            onChange={(v:string) => F("bag_horoo",v)} editing={editing}/>
          <FInput label="Тоот" value={form.toot}
            onChange={(v:string) => F("toot",v)} editing={editing} placeholder="Байрны тоот"/>
        </div>
        <div style={{ marginBottom:12 }}>
          <FRadio label="Оршин суугаа хаяг бүртгэлийнхээс өөр үү?"
            value={form.address_different}
            onChange={(v:any) => F("address_different", v === "true" || v === true)}
            editing={editing}
            options={[{value:true,label:"Тийм, өөр"},{value:false,label:"Үгүй, адил"}]}/>
        </div>
        {form.address_different && (
          <div style={{ animation:"slideIn .2s ease" }}>
            <FInput label="Оршин суугаа хаяг" value={form.orshisuugaa_hayag}
              onChange={(v:string) => F("orshisuugaa_hayag",v)} editing={editing}
      ...
      {/* ── 3. Үйл ажиллагаа ──────────────────────────────────── */}
      <Card>
        <SectionTitle icon={Briefcase} title="Үйл ажиллагаа" subtitle="Нийлүүлэх чиглэл болон туршлага"/>
              ...
              return (
                <button key={d.id} type="button"
                  ...
                  style={{ padding:"7px 16px", borderRadius:99, fontSize:12, fontFamily:"inherit",
                    ...
                    fontWeight: on ? 600 : 400, transition:"all .12s" }}>{d.label}</button>
              );
            })}
          </div>
          {!editing && selDirs.length === 0 && (
            <div style={{ fontSize:13, color:"#cbd5e1", padding:"10px 0",
              borderBottom:"1px solid #f1f5f9" }}>—</div>
          )}
        </div>
        <div style={{ marginBottom:16 }}>
        ...
        <div>
          ...
          {editing ? (
            <textarea value={form.activity_description}
              ...
              rows={3} placeholder="Хийж буй үйл ажиллагаа, туршлагаа товч тайлбарлана уу..."
              style={{ width:"100%", padding:"10px 14px", borderRadius:10,
                border:"1.5px solid #e2e8f0", fontSize:13, color:"#1e293b",
      ...
      <Card>
        ...
        <div className="profile-docs"
          ...
          <DocCard label="Профайл зураг" fieldKey="profile_photo"
            preview={previews.profile_photo} onFile={onFile} editing={editing}
            accept="image/*" required/>
          <DocCard label="Иргэний үнэмлэх (урд)" fieldKey="id_card_front"
            preview={previews.id_card_front} onFile={onFile} editing={editing}
            accept="image/*" required/>
          <DocCard label="Иргэний үнэмлэх (ард)" fieldKey="id_card_back"
            preview={previews.id_card_back} onFile={onFile} editing={editing}
            accept="image/*"/>
      ...
      <Card>
        <SectionTitle icon={Bell} title="Мэдэгдлийн тохиргоо"
        ...
        <FSelect label="Мэдэгдэл хүлээн авах арга" value={form.notification_type}
          ...
          options={[
            ...
            {value:"sms",   label:"SMS-ээр"},
            {value:"both",  label:"И-мэйл + SMS хоёулаа"},
