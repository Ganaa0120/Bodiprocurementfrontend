...
import {
  Pencil, X, Check, Upload, Loader2, AlertCircle, ChevronDown,
  User, MapPin, Briefcase, FileText, Bell, CheckCircle, Camera,
  Shield, Search, ChevronRight,
} from "lucide-react";
import { validateMongolianForm, isMongolian } from "@/utils/mongolianValidation";
...
const BLANK = {
  family_name:"", last_name:"", first_name:"", birth_date:"", gender:"", phone:"",
  aimag_niislel:"", sum_duureg:"", bag_horoo:"", toot:"", address_different:false,
  orshisuugaa_hayag:"", activity_description:"", activity_start_date:"",
  is_vat_payer:false, notification_type:"email", notification_preference:"all",
};
...
function fmtDate(v?: string | null) {
  if (!v) return null;
  try { return new Date(v).toLocaleDateString("mn-MN",{year:"numeric",month:"2-digit",day:"2-digit"}); }
  catch { return v; }
}

function calcPct(form:any, selDirs:SelDir[], previews:Record<string,string>) {
  const scored = [
    {v:form.last_name,w:8},{v:form.first_name,w:8},{v:form.family_name,w:4},
    {v:form.birth_date,w:8},{v:form.gender,w:6},{v:form.phone,w:8},
    {v:form.aimag_niislel,w:6},{v:form.sum_duureg,w:4},{v:form.bag_horoo,w:4},
    {v:form.toot,w:2},{v:form.activity_description,w:4},{v:form.activity_start_date,w:3},
    {v:selDirs.length>0,w:10},{v:previews.profile_photo,w:10},
    {v:previews.id_card_front,w:10},{v:previews.id_card_back,w:8},{v:previews.activity_intro,w:5},
  ];
...
const S = {
  label:{
    ...
    color:"#94a3b8", display:"block" as const, marginBottom:6,
  ...
  return (
    <div>
      ...
      {editing ? (
        <>
          <input type={type} value={value} onChange={e=>onChange(e.target.value)}
            ...
            style={{ width:"100%", padding:"10px 14px", borderRadius:10,
              ...
              background:fieldError?"#fff5f5":"white", boxSizing:"border-box" as const,
        ...
        <div style={{ fontSize:13, color:display?"#1e293b":"#cbd5e1", padding:"10px 0",
          borderBottom:"1px solid #f1f5f9", fontWeight:display?500:400,
      ...
      {editing ? (
        <div style={{ position:"relative" }}>
          <select value={value} onChange={e=>onChange(e.target.value)}
            style={{ width:"100%", padding:"10px 36px 10px 14px", borderRadius:10,
              border:"1.5px solid #e2e8f0", fontSize:13, color:value?"#1e293b":"#94a3b8",
        ...
        <div style={{ fontSize:13, color:value?"#1e293b":"#cbd5e1", padding:"10px 0",
          borderBottom:"1px solid #f1f5f9", fontWeight:value?500:400 }}>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" as const }}>
        {options.map((o:any)=>{
          ...
          return editing ? (
            ...
            <span key={String(o.value)} style={{ fontSize:13, padding:"9px 0",
  ...
  return (
    <div>
      ...
      <div onClick={()=>editing&&inputRef.current?.click()}
        style={{ borderRadius:12, border:preview?"1.5px solid #a7f3d0":"1.5px dashed #e2e8f0",
          background:preview?"#f0fdf4":"#fafafa", minHeight:90,
          display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden",
          position:"relative", transition:"all .15s", cursor:editing?"pointer":"default",
...
function Card({ children, style }:{ children:React.ReactNode; style?:React.CSSProperties }) {
  return (
    <div style={{ background:"white", borderRadius:18, border:"1px solid #f1f5f9",
  ...
  return (
    <svg width={72} height={72} viewBox="0 0 72 72">
      ...
      <circle cx={36} cy={36} r={r} fill="none" stroke={color} strokeWidth={5}
        ...
        transform="rotate(-90 36 36)" style={{ transition:"stroke-dasharray .8s ease" }}/>
      <text x={36} y={40} textAnchor="middle" fontSize={13} fontWeight={700} fill={color}>{pct}%</text>
    </svg>
  );
}

// ✅ Шинэ DirectionPicker — 40+ чиглэлийг хялбар сонгох
function DirectionPicker({ dirs, selDirs, editing, toggleMain, toggleSub }:{
  ...
  const filtered = dirs.filter(d =>
    ...
    d.children.some(c => c.label.toLowerCase().includes(search.toLowerCase()))
  );

  const selectedCount = selDirs.length;
  const totalSubSelected = selDirs.reduce((s,d)=>s+d.sub_ids.length,0);

  // Read-only view
  if (!editing) {
    if (selDirs.length === 0) return (
      <div style={{ fontSize:13, color:"#cbd5e1", padding:"10px 0", borderBottom:"1px solid #f1f5f9" }}>—</div>
    );
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {selDirs.map(sel => {
          ...
          if (!main) return null;
          return (
            <div key={sel.main_id} style={{ padding:"10px 14px", borderRadius:12,
              ...
              {sel.sub_ids.length>0&&(
                <div style={{ display:"flex", flexWrap:"wrap", gap:5, paddingLeft:36 }}>
                  {sel.sub_ids.map(sid=>{
                    ...
                    return sub?(
                      <span key={sid} style={{ fontSize:11, color:"#4f46e5", background:"#eef2ff",
                        border:"1px solid #c7d2fe", padding:"2px 10px", borderRadius:99 }}>
                        {sub.label}
                      </span>
                    ):null;
                  })}
  ...
  return (
    <div>
      {/* ✅ Хэдэн чиглэл сонгосон summary */}
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12, flexWrap:"wrap" as const }}>
        <div style={{ fontSize:13, color:"#64748b" }}>
          {selectedCount===0
            ? <span style={{ color:"#f59e0b", fontWeight:500 }}>⚠️ Нэг буюу хэд хэдэн чиглэл сонгоно уу</span>
            : <span style={{ color:"#059669", fontWeight:600 }}>
                ✓ {selectedCount} үндсэн, {totalSubSelected} дэд чиглэл сонгогдсон
              </span>}
        </div>
        {selectedCount>0&&(
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:6,
        ...
        {filtered.length===0 ? (
          <div style={{ textAlign:"center" as const, padding:"24px 0", color:"#94a3b8", fontSize:13 }}>
            "{search}" хайлтанд тохирох чиглэл олдсонгүй
          </div>
        ) : filtered.map(d=>{
          const sel    = selDirs.find(s=>Number(s.main_id)===Number(d.id));
          const isOn   = !!sel;
          const isOpen = expanded===d.id;
          const selectedSubCount = sel?.sub_ids.length ?? 0;

          // Search-д таарсан дэд чиглэлүүд
          const matchedChildren = search
            ? d.children.filter(c=>c.label.toLowerCase().includes(search.toLowerCase()))
            : d.children;

          return (
            <div key={d.id} style={{ borderRadius:12, overflow:"hidden",
              border:isOn?"1.5px solid #6366f1":"1px solid #e2e8f0",
              background:isOn?"#f8f9ff":"white", transition:"all .15s" }}>

              {/* Үндсэн мөр */}
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 14px" }}>
                {/* Checkbox */}
                <div onClick={()=>toggleMain(d.id)}
                ...
                <div style={{ width:32, height:32, borderRadius:8, flexShrink:0,
                  background:isOn?"#6366f1":"#f1f5f9",
                  display:"flex", alignItems:"center", justifyContent:"center",
                  fontSize:13, fontWeight:800, color:isOn?"white":"#94a3b8",
                  transition:"all .15s" }}>
                  {d.label[0]}
                </div>

                {/* Нэр */}
                <div style={{ flex:1, minWidth:0, cursor:"pointer" }} onClick={()=>toggleMain(d.id)}>
                  <div style={{ fontSize:13, fontWeight:600, color:isOn?"#4f46e5":"#0f172a",
                    overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" as const }}>
                    {d.label}
                  </div>
                  {d.children.length>0&&(
                    <div style={{ fontSize:11, color:"#94a3b8", marginTop:1 }}>
                      {isOn&&selectedSubCount>0
                        ? <span style={{ color:"#6366f1" }}>
                            {selectedSubCount}/{d.children.length} дэд чиглэл сонгогдсон
                          </span>
                ...
                {d.children.length>0&&(
                  <button type="button"
                    ...
                    style={{ padding:"5px 10px", borderRadius:8,
                      ...
                      background:isOn?"#eef2ff":"#fafafa",
                      cursor:"pointer", display:"flex", alignItems:"center", gap:4,
                      fontSize:11, color:isOn?"#6366f1":"#64748b", fontFamily:"inherit",
                      whiteSpace:"nowrap" as const, flexShrink:0 }}>
                    <ChevronDown size={11}
                      style={{ transform:isOpen?"rotate(180deg)":"rotate(0deg)", transition:"transform .2s" }}/>
                    {isOpen?"Хураах":"Дэд чиглэл"}
                  </button>
                )}
              </div>

              {/* ✅ Дэд чиглэлүүд */}
              {(isOpen||(search&&matchedChildren.length>0))&&d.children.length>0&&(
                <div style={{ padding:"0 14px 12px 68px",
                  borderTop:`1px solid ${isOn?"#e8eaff":"#f1f5f9"}`,
                  background:isOn?"rgba(99,102,241,0.02)":"#fafafa" }}>
                  <div style={{ fontSize:10, fontWeight:700, color:"#94a3b8",
                    letterSpacing:"0.08em", textTransform:"uppercase" as const, margin:"10px 0 8px" }}>
                    Дэд чиглэл сонгох
                  </div>
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                    {(search?matchedChildren:d.children).map(sub=>{
                      const isSubOn = sel?.sub_ids.some(id=>Number(id)===Number(sub.id))??false;
                      return (
                        <button key={sub.id} type="button" onClick={()=>toggleSub(d.id,sub.id)}
                          style={{ padding:"5px 12px", borderRadius:8, fontSize:12, fontWeight:500,
                            border:isSubOn?"1.5px solid #6366f1":"1.5px solid #e2e8f0",
...
export default function PersonProfilePage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/activity-directions`).then(r=>r.json()).then(d=>{
      if(d.success) setDirs((d.directions||[]).map((dir:any)=>({
  ...
  const handleSave = async () => {
    ...
    if(form.bag_horoo&&!/^[\u0400-\u04FF\s\d\-\/\.]+$/.test(form.bag_horoo)){
      setFieldErrors(p=>({...p,bag_horoo:"Крилл үсэг, тоо оруулна уу"}));
      setError("Баг/Хороо талбар буруу байна"); return;
    }
    if(form.phone&&form.phone.length!==8){
      setFieldErrors(p=>({...p,phone:"8 оронтой тоо оруулна уу"}));
      setError("Утасны дугаар 8 оронтой байх ёстой"); return;
    }

    setFieldErrors({}); setSaving(true); setError("");
    const token = localStorage.getItem("token");
    if(!token) return;
    ...
    <div style={{ maxWidth:860, margin:"0 auto",
          ...
          <div style={{ display:"flex", gap:8, justifyContent:isMobile?"flex-end":"flex-start" }}>
            ...
            <button onClick={handleSave} disabled={saving}
              style={{ display:"flex", alignItems:"center", gap:5, padding:"8px 20px",
                ...
                opacity:saving?0.7:1, flex:isMobile?1:"none" as any,
      ...
      {error&&(
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px",
          borderRadius:12, background:"#fef2f2", border:"1px solid #fecaca", animation:"fadeIn .3s ease" }}>
      ...
      {saved&&(
        <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px",
          borderRadius:12, background:"#f0fdf4", border:"1px solid #a7f3d0", animation:"fadeIn .3s ease" }}>
      ...
      {/* ── Hero ── */}
      <Card>
        <div style={{ display:"flex", flexDirection:isMobile?"column":"row" as any,
          alignItems:isMobile?"flex-start":"center", gap:20 }}>
          <label style={{ cursor:editing?"pointer":"default", flexShrink:0, position:"relative" }}>
            <div style={{ width:76, height:76, borderRadius:20, overflow:"hidden",
              background:previews.profile_photo?"transparent":"linear-gradient(135deg,#6366f1,#8b5cf6)",
              display:"flex", alignItems:"center", justifyContent:"center", border:"3px solid #f1f5f9" }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" as const }}>
              {profile?.supplier_number&&(
                <span style={{ fontSize:11, padding:"3px 10px", borderRadius:8,
                  background:"#f8fafc", color:"#475569", fontFamily:"monospace" }}>
                  {profile.supplier_number}
                </span>
              )}
              {profile?.email&&<span style={{ fontSize:12, color:"#64748b" }}>{profile.email}</span>}
            </div>
          </div>
            ...
            {!editing&&!isNewUser&&(
              <button onClick={startEdit}
                style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 16px",
                  ...
                  color:"#0f172a", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}
      ...
      {/* ── 1. Үндсэн мэдээлэл ── */}
      <Card>
        <SectionTitle icon={User} title="Үндсэн мэдээлэл" subtitle="Хувийн болон холбоо барих"/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:16, marginBottom:16 }}>
          <FInput label="Төрсөн огноо *" type="date" value={form.birth_date}
            onChange={(v:string)=>F("birth_date",v)} editing={editing}/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:16 }}>
          <FInput label="Утасны дугаар" type="tel" value={form.phone} editing={editing}
            onChange={(v:string)=>{ const d=v.replace(/\D/g,"").slice(0,8); F("phone",d);
              setFieldErrors(p=>({...p,phone:d&&d.length<8?"8 оронтой тоо оруулна уу":""})); }}
      ...
      {/* ── 2. Хаяг ── */}
      <Card>
        <SectionTitle icon={MapPin} title="Хаяг, байршил" subtitle="Бүртгэлийн болон оршин суугаа хаяг"/>
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:16, marginBottom:16 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:16, marginBottom:16 }}>
          {form.aimag_niislel==="Улаанбаатар"&&form.sum_duureg&&UB_DUUREG[form.sum_duureg]
            ...
            : <FInput label="Баг / Хороо *" value={form.bag_horoo} editing={editing}
                onChange={(v:string)=>{ F("bag_horoo",v);
                  setFieldErrors(p=>({...p,bag_horoo:v&&!/^[\u0400-\u04FF\s\d\-\/\.]+$/.test(v)?"Крилл үсэг, тоо оруулна уу":""})); }}
                fieldError={fieldErrors.bag_horoo}/>}
          <FInput label="Байр, тоот" value={form.toot} editing={editing}
            onChange={(v:string)=>F("toot",v)} placeholder="Байр болон тоот"/>
        </div>
        <div style={{ marginBottom:12 }}>
          <FRadio label="Оршин суугаа хаяг бүртгэлийнхээс өөр үү?"
            value={form.address_different}
            onChange={(v:any)=>F("address_different",v==="true"||v===true)} editing={editing}
            options={[{value:true,label:"Тийм, өөр"},{value:false,label:"Үгүй, адил"}]}/>
        </div>
        {form.address_different&&(
          <div style={{ animation:"slideIn .2s ease" }}>
            <FInput label="Оршин суугаа хаяг" value={form.orshisuugaa_hayag} editing={editing}
      ...
      {/* ── 3. Үйл ажиллагаа ── */}
      <Card>
        <SectionTitle icon={Briefcase} title="ЭРХЭЛЖ БУЙ ҮЙЛ АЖИЛЛАГААНЫ МЭДЭЭЛЭЛ"
          subtitle="Та аль салбарт нийлүүлэлт хийдэг вэ?"/>

        {/* Заавар */}
        {editing&&(
          <div style={{ padding:"10px 14px", borderRadius:10, background:"#f8f9ff",
            border:"1px solid #e8eaff", marginBottom:14,
            display:"flex", alignItems:"flex-start", gap:8 }}>
            <span style={{ fontSize:16, flexShrink:0 }}>💡</span>
            <div style={{ fontSize:12, color:"#4f46e5", lineHeight:1.6 }}>
              Өөрийн бизнест тохирох нэг буюу хэд хэдэн чиглэл сонгоно уу.
              Дэд чиглэлийг нэмж тодруулж болно. Хайлтаар хурдан олно уу.
            </div>
          </div>
        )}
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ ...S.label, marginBottom:10 }}>
            Нийлүүлэх чиглэл {editing&&<span style={{ color:"#ef4444" }}>*</span>}
          </label>
          <DirectionPicker
            dirs={dirs} selDirs={selDirs} editing={editing}
            toggleMain={toggleMain} toggleSub={toggleSub}/>
        </div>

        <div style={{ marginBottom:16 }}>
          <FInput label="Үйл ажиллагаа эхэлсэн огноо *" type="date"
            value={form.activity_start_date} onChange={(v:string)=>F("activity_start_date",v)} editing={editing}/>
        </div>
      ...
      {/* ── 4. Баримт бичиг ── */}
      <Card>
        <SectionTitle icon={Shield} title="Баримт бичиг" subtitle="Таниулах бичиг баримт"/>
        <div style={{ display:"grid", gridTemplateColumns:gDoc, gap:12 }}>
          <DocCard label="Профайл зураг" fieldKey="profile_photo" preview={previews.profile_photo}
            onFile={onFile} editing={editing} accept="image/*" required/>
          <DocCard label="Иргэний үнэмлэх (урд)" fieldKey="id_card_front" preview={previews.id_card_front}
            onFile={onFile} editing={editing} accept="image/*" required/>
          <DocCard label="Иргэний үнэмлэх (ард)" fieldKey="id_card_back" preview={previews.id_card_back}
            onFile={onFile} editing={editing} accept="image/*" required/>
          <DocCard label="Үйл ажиллагааны танилцуулга" fieldKey="activity_intro" preview={previews.activity_intro}
            onFile={onFile} editing={editing}
      ...
      {/* ── 5. Мэдэгдэл ── */}
      <Card>
        <SectionTitle icon={Bell} title="Мэдэгдлийн тохиргоо" subtitle="Зарын мэдэгдэл хэрхэн хүлээн авах"/>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {[
            { value:"all", icon:"🔔", label:"Бүх чиглэлийн зарыг хүлээн авах",
              desc:"Системд нийтлэгдсэн бүх зарын мэдэгдлийг хүлээн авна" },
            { value:"selected_dirs", icon:"🎯", label:"Зөвхөн миний чиглэлийн зарыг хүлээн авах",
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", gap:10,
          ...
          <button onClick={handleSave} disabled={saving}
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6,
              ...
              fontSize:13, fontWeight:600, cursor:"pointer", opacity:saving?0.7:1,
              fontFamily:"inherit", boxShadow:"0 4px 14px rgba(99,102,241,0.35)",
