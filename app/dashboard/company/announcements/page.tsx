...
export default function CompanyAnnouncementsPage() {
      ...
      {(selected || detLoading) && (
        <div style={{ position:"fixed",inset:0,zIndex:100,background:"rgba(0,0,0,0.55)",
          ...
          <div style={{ width:"100%",maxWidth:640,background:"white",borderRadius:24,
            boxShadow:"0 32px 80px rgba(0,0,0,0.25)",animation:"modalIn .25s cubic-bezier(0.34,1.56,0.64,1)",
  ...
  return (
    <>
      {/* ── Hero header ── */}
      <div style={{ padding:"24px 26px 20px",borderBottom:"1px solid #f1f5f9" }}>
        <div style={{ display:"flex",alignItems:"flex-start",gap:14,marginBottom:14 }}>
          ...
          <div style={{ flex:1,minWidth:0 }}>
            ...
            <h2 style={{ fontSize:18,fontWeight:700,color:"#0f172a",margin:"0 0 8px",lineHeight:1.3 }}>
              {selected.title}
            </h2>
          </div>
          <button onClick={() => setSelected(null)}
            style={{ background:"#f8fafc",border:"1px solid #f1f5f9",borderRadius:10,
        ...
        <div style={{ display:"flex",gap:8,flexWrap:"wrap" as const }}>
          {selected.deadline && (
            <div style={{ display:"flex",alignItems:"center",gap:6,padding:"6px 12px",
              borderRadius:99,background:isExpired?"#fef2f2":"#f0fdf4",
          ...
          {selected.category_name && (
            <div style={{ display:"flex",alignItems:"center",gap:6,padding:"6px 12px",
              borderRadius:99,background:"#f8fafc",border:"1px solid #e2e8f0",
      ...
      {/* ── Body ── */}
      <div style={{ padding:"22px 26px",display:"flex",flexDirection:"column" as const,gap:20 }}>
        ...
        {selected.description && (
          <div>
            ...
            <div style={{ fontSize:14,color:"#334155",lineHeight:1.85,background:"#fafafa",
        ...
        {selected.requirements && (
          <div>
            ...
            <div style={{ background:"#fffbeb",borderRadius:14,padding:"16px 18px",
          ...
          <div>
            ...
            <div style={{ display:"flex",flexWrap:"wrap" as const,gap:7 }}>
              {selected.activity_directions.map((d: string) => (
                <span key={d} style={{ fontSize:12,padding:"6px 14px",borderRadius:99,
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",
          ...
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            {selected.created_by_name ? (
              <>
                <div style={{ width:30,height:30,borderRadius:9,background:"#eef2ff",
                  display:"flex",alignItems:"center",justifyContent:"center",
      ...
      {/* ── Footer ── */}
      <div style={{ padding:"0 26px 24px",display:"flex",
        ...
        <button onClick={() => { setBidModal(true); setBidError(""); }}
      ...
      {/* ── Bid Modal ── */}
      {bidModal && selected && (
        <div style={{ position:"fixed",inset:0,zIndex:110,background:"rgba(0,0,0,0.6)",
          backdropFilter:"blur(6px)",display:"flex",alignItems:"center",
          justifyContent:"center",padding:"20px 16px" }}
          onClick={() => setBidModal(false)}>
          <div style={{ width:"100%",maxWidth:440,background:"white",borderRadius:20,
            ...
            <div style={{ padding:"20px 22px",borderBottom:"1px solid #f1f5f9",
              ...
              <div style={{ flex:1,minWidth:0 }}>
                ...
                <div style={{ fontSize:11,color:"#94a3b8",marginTop:1,overflow:"hidden",
                  textOverflow:"ellipsis",whiteSpace:"nowrap" as const }}>{selected.title}</div>
              </div>
              <button onClick={() => setBidModal(false)}
                style={{ background:"#f8fafc",border:"1px solid #f1f5f9",borderRadius:9,
                ...
                <>
                  {bidError && (
                    <div style={{ padding:"10px 14px",borderRadius:10,background:"#fef2f2",
                      border:"1px solid #fecaca",fontSize:13,color:"#dc2626",marginBottom:16 }}>
                      {bidError}
                    </div>
                  )}
                  <div style={{ padding:"14px 16px",borderRadius:12,background:"#f8fafc",
                    ...
                    <div style={{ display:"flex",flexDirection:"column" as const,gap:6 }}>
                      {[
                        { label:"Байгааллага", value: user?.company_name },
                        ...
                        <div key={row.label} style={{ display:"flex",gap:8 }}>
                          ...
                          <span style={{ fontSize:12,fontWeight:500,color:"#0f172a" }}>{row.value}</span>
                        </div>
                      ))}
                    </div>
                    <div style={{ marginTop:10,fontSize:11,color:"#94a3b8",padding:"8px 10px",
                  ...
                  <div style={{ display:"flex",gap:10 }}>
                    ...
                    <button onClick={submitBid} disabled={bidSaving}
      ...
      <div style={{ marginBottom:20 }}>
        <h1 style={{ fontSize:20,fontWeight:700,color:"#0f172a",margin:"0 0 4px" }}>Зарлалууд</h1>
        ...
        {TABS.map(({ key, label }) => {
          ...
          return (
            <button key={key} onClick={() => setTypeF(key)}
              style={{ padding:"8px 14px",borderRadius:10,cursor:"pointer",fontSize:12,
                fontWeight:500,fontFamily:"inherit",display:"flex",alignItems:"center",gap:6,
                border: active ? `1.5px solid ${tc?.color ?? "#6366f1"}` : "1.5px solid #e2e8f0",
              ...
              {cnt > 0 && (
                <span style={{ fontSize:10,fontWeight:700,padding:"1px 6px",borderRadius:99,
                  ...
                  color: active ? "white" : "#64748b" }}>{cnt}</span>
              )}
            </button>
          );
        })}
        <div style={{ marginLeft:"auto",display:"flex",gap:8 }}>
          <div style={{ position:"relative" }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Хайх..."
              style={{ padding:"8px 12px 8px 32px",borderRadius:10,border:"1.5px solid #e2e8f0",
                fontSize:12,outline:"none",background:"white",width:180,
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center" as const,padding:"56px 0",background:"white",
            ...
            return (
              <div key={a.id} onClick={() => openDetail(a.id)}
                style={{ background:"white",borderRadius:16,padding:"18px 20px",
                  ...
                  boxShadow:"0 1px 4px rgba(0,0,0,0.04)",cursor:"pointer",transition:"all .15s" }}
                ...
                <div style={{ display:"flex",alignItems:"flex-start",gap:14 }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    ...
                    {a.description && (
                      <p style={{ fontSize:12,color:"#64748b",margin:"0 0 6px",lineHeight:1.5,
                        ...
                        {a.description.replace(/<[^>]*>/g," ").replace(/\s+/g," ").trim().slice(0,120)}
                      </p>
                    )}
                  ...
                  <div style={{ flexShrink:0,textAlign:"right" as const }}>
                    ...
                    <div style={{ marginTop:8,fontSize:11,fontWeight:600,color:tc.color,
