    ...
    <div style={{ maxWidth:"100%", margin:"0 auto", padding:"24px 16px" }}>
      ...
      {selected && (
        <div style={{ position:"fixed",inset:0,zIndex:100,background:"rgba(0,0,0,0.5)",
          ...
          <div style={{ width:"100%",maxWidth:520,background:"white",borderRadius:24,
            boxShadow:"0 24px 60px rgba(0,0,0,0.2)",animation:"modalIn .25s cubic-bezier(0.34,1.56,0.64,1)",
            maxHeight:"80vh",overflowY:"auto" }}
            onClick={e => e.stopPropagation()}>

            <div style={{ display:"flex",alignItems:"flex-start",gap:14,
              padding:"22px 24px",borderBottom:"1px solid #f1f5f9" }}>
              <div style={{ width:46,height:46,borderRadius:14,flexShrink:0,background:"#eef2ff",
                display:"flex",alignItems:"center",justifyContent:"center",fontSize:22 }}>
                {TYPE_ICON[selected.notification_type ?? "system"] ?? "🔔"}
              </div>
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ fontSize:15,fontWeight:700,color:"#0f172a",marginBottom:5,lineHeight:1.4 }}>
            ...
            <div style={{ padding:"22px 24px" }}>
              {selected.image_url && (
                <div style={{ marginBottom:18,borderRadius:14,overflow:"hidden",
                  ...
                  <img src={selected.image_url} alt="" style={{ width:"100%",maxHeight:280,
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        <div>
          <h1 style={{ fontSize:20,fontWeight:700,color:"#0f172a",margin:"0 0 4px" }}>
            Мэдэгдэл
            {unread > 0 && (
              <span style={{ marginLeft:8,fontSize:12,fontWeight:700,padding:"2px 8px",
                borderRadius:99,background:"#6366f1",color:"white",verticalAlign:"middle" }}>
                {unread}
              </span>
            )}
          </h1>
          ...
          {notifs.map(n => (
            <div key={n.id} onClick={() => openNotif(n)}
              style={{ background:"white",borderRadius:14,padding:"14px 18px",
                ...
                opacity:n.is_read?0.7:1,transition:"all .15s",
                boxShadow:n.is_read?"none":"0 2px 8px rgba(99,102,241,0.08)" }}
              ...
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ fontSize:13,fontWeight:n.is_read?400:600,color:"#0f172a",
                ...
                {n.message && n.title && (
                  <div style={{ fontSize:11,color:"#64748b",overflow:"hidden",
                    textOverflow:"ellipsis",whiteSpace:"nowrap" as const,marginBottom:2 }}>
                    {n.message}
                  </div>
                )}
              ...
              {!n.is_read && (
                <div style={{ width:9,height:9,borderRadius:"50%",background:"#6366f1",
                  flexShrink:0,boxShadow:"0 0 6px rgba(99,102,241,0.5)" }}/>
              )}
            </div>
          ))}
        </div>
