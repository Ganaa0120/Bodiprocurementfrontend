  ...
  return (
    <button type="button" onClick={onClick}
      style={{ display:"flex", alignItems:"center", justifyContent:"center",
        gap:6, padding:12, borderRadius:12, border:"1.5px dashed #c7d2fe",
        background:"#eef2ff", color:"#6366f1", fontSize:13, fontWeight:600,
        cursor:"pointer", width:"100%", transition:"all .15s", fontFamily:"inherit" }}
...
function OwnerCard({ idx, label, editing, owner, fieldErrors, canRemove, onRemove, onUpdate }: any) {
  const w = useW();
  ...
  const gOwner = isFirst
    ? isMobile ? "1fr" : isTablet ? "1fr 1fr" : "1fr 1fr 1fr 1fr 1fr"
      ...
      <div style={{ display:"grid", gridTemplateColumns:gOwner, gap:14 }}>
        {isFirst ? (
          <>
            ...
            <FSelect label="Хүйс" value={owner.gender} editing={editing}
            ...
            <FInput label="Утас" value={owner.phone} editing={editing}
              onChange={(v:string)=>onUpdate(idx,"phone",v.replace(/\D/g,"").slice(0,8))}
    ...
    <Section icon={User} title="ӨМЧЛӨГЧИЙН МЭДЭЭЛЭЛ">
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {owners.map((owner:any, idx:number) => (
          <OwnerCard key={idx} idx={idx} label="эзэмшигч"
            editing={editing} owner={owner} fieldErrors={fieldErrors}
            canRemove={owners.length>1}
            onRemove={()=>setOwners((p:any[])=>p.filter((_,i)=>i!==idx))}
            onUpdate={update}/>
        ))}
        {editing && (
          <AddBtn onClick={()=>setOwners((p:any[])=>[...p,{...BLANK_OWNER}])}
...
export function ExecutiveDirectorsSection({
  directors, setDirectors, editing,
  ...
  const update = (idx:number, key:string, val:string) => {
    ...
    if (["position","last_name","first_name"].includes(key)) {
      if (val && !/^[\u0400-\u04FF\s\-]+$/.test(val)) msg = "Крилл үсгээр бичнэ үү";
    }
    if (key==="phone") {
      if (val && (!/^\d+$/.test(val)||val.length!==8)) msg = "8 оронтой тоо оруулна уу";
    }
    if (key==="email") {
      if (val && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) msg = "И-мэйл хаяг буруу байна";
    }
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {directors.map((d:any, idx:number) => (
          <div key={idx} style={{ padding:16, borderRadius:12,
            ...
            <div style={{ display:"grid", gridTemplateColumns:gDir, gap:14 }}>
              ...
              <FInput label="Утас" value={d.phone} editing={editing}
                onChange={(v:string)=>update(idx,"phone",v.replace(/\D/g,"").slice(0,8))}
                fieldError={fieldErrors[`${idx}_phone`]}/>
