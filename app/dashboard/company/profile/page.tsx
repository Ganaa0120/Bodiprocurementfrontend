...
import { Loader2, Building2, MapPin, Briefcase, FileText, CreditCard } from "lucide-react";
import { API, BLANK, BLANK_OWNER, BLANK_FINAL, AIMAG } from "./_components/constants";
import { SaveBar, HeroCard, Alert } from "./_components/HeroCard";
...
export default function CompanyProfilePage() {
        ...
        .then(d => {
          if (d.success && (d.organization || d.user)) {
            ...
            localStorage.setItem("user", JSON.stringify({
              ...stored, status:fresh.status, return_reason:fresh.return_reason
            }));
          }
        })
        .catch(() => {});
  ...
  return (
    <div style={{ maxWidth:900,margin:"0 auto",display:"flex",flexDirection:"column",
      ...
      {editing && <SaveBar saving={saving} onSave={handleSave} onCancel={cancelEdit}/>}

      <HeroCard profile={profile} previews={previews} editing={editing}
        onFile={onFile} startEdit={startEdit} pct={pct}/>
      ...
      <Section icon={Building2} title="Байгааллагын мэдээлэл">
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14 }}>
          <FInput label="Байгааллагын нэр *" value={form.company_name}   editing={editing} onChange={(v:string)=>F("company_name",v)}/>
          <FInput label="Гадаад нэр"          value={form.company_name_en} editing={editing} onChange={(v:string)=>F("company_name_en",v)}/>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14 }}>
          <FInput label="Регистрийн дугаар *"     value={form.register_number}       editing={editing} onChange={(v:string)=>F("register_number",v)}       mono/>
          <FInput label="Улсын бүртгэлийн дугаар" value={form.state_registry_number} editing={editing} onChange={(v:string)=>F("state_registry_number",v)} mono/>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:14 }}>
          <FSelect label="Байгааллагын хэлбэр" value={form.company_type}     editing={editing} onChange={(v:string)=>F("company_type",v)}     options={["ХХК","ХК","ТББ","Бусад"]}/>
          <FInput  label="Байгуулагдсан огноо"  value={form.established_date} editing={editing} onChange={(v:string)=>F("established_date",v)} type="date"/>
          <FInput  label="Ажилчдын тоо"         value={form.employee_count}   editing={editing} onChange={(v:string)=>F("employee_count",v)}   placeholder="0"/>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14 }}>
          <FInput     label="Утас"          value={form.phone}           editing={editing} onChange={(v:string)=>F("phone",v)}/>
          <RadioGroup label="НӨАТ төлөгч"   value={form.is_vat_payer}    editing={editing} onChange={(v:any)=>F("is_vat_payer",v)}    options={[{value:true,label:"Тийм"},{value:false,label:"Үгүй"}]}/>
          <RadioGroup label="ISO гэрчилгээ" value={form.is_iso_certified} editing={editing} onChange={(v:any)=>F("is_iso_certified",v)} options={[{value:true,label:"Тийм"},{value:false,label:"Үгүй"}]}/>
        </div>
      </Section>
      ...
      <Section icon={MapPin} title="Хаяг">
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14 }}>
          <FSelect label="Аймаг / Нийслэл" value={form.aimag_niislel} editing={editing} onChange={(v:string)=>F("aimag_niislel",v)} options={AIMAG} placeholder="Сонгох"/>
          <FInput  label="Сум / Дүүрэг"    value={form.sum_duureg}    editing={editing} onChange={(v:string)=>F("sum_duureg",v)}/>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 2fr",gap:14 }}>
          <FInput label="Баг / Хороо"      value={form.bag_horoo} editing={editing} onChange={(v:string)=>F("bag_horoo",v)}/>
          <FInput label="Дэлгэрэнгүй хаяг" value={form.address}   editing={editing} onChange={(v:string)=>F("address",v)}/>
        </div>
      </Section>
      ...
      <Section icon={Briefcase} title="Үйл ажиллагаа">
            ...
            {dirs.map(({ id, label }) => {
              ...
              return (
                <button key={id} type="button"
                  onClick={() => editing && setSelDirs(p => p.includes(id) ? p.filter(d=>d!==id) : [...p,id])}
                  style={{ padding:"5px 14px",borderRadius:99,fontSize:12,fontWeight:500,
                    border: on ? "1.5px solid #6366f1" : "1.5px solid #e2e8f0",
                    background: on ? "#eef2ff" : "white",color: on ? "#4f46e5" : "#64748b",
                    cursor: editing ? "pointer" : "default",transition:"all .12s" }}>
                  {label}
                </button>
              );
            })}
        ...
        {form.has_special_permission && (
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,
            padding:14,borderRadius:12,background:"#f8fafc",border:"1px solid #f1f5f9" }}>
            <FInput label="Тусгай зөвшөөрлийн дугаар" value={form.special_permission_number}
              editing={editing} onChange={(v:string)=>F("special_permission_number",v)}/>
            <FInput label="Дуусах огноо" type="date"   value={form.special_permission_expiry}
              editing={editing} onChange={(v:string)=>F("special_permission_expiry",v)}/>
          </div>
        )}
      </Section>
      ...
      <FinalOwnersSection finalOwners={finalOwners} setFinalOwners={setFinalOwners} editing={editing}/>

      {/* 6. Баримт бичиг */}
      <Section icon={FileText} title="Баримт бичиг">
        <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14 }}>
          <DocUpload label="Улсын бүртгэлийн гэрчилгээ" fieldKey="doc_state_registry"    preview={previews.doc_state_registry}    onFile={onFile} editing={editing} accept=".pdf,image/*" required/>
          <DocUpload label="НӨАТ-ын гэрчилгээ"           fieldKey="doc_vat_certificate"   preview={previews.doc_vat_certificate}   onFile={onFile} editing={editing} accept=".pdf,image/*"/>
          <DocUpload label="Тусгай зөвшөөрөл"            fieldKey="doc_special_permission" preview={previews.doc_special_permission} onFile={onFile} editing={editing} accept=".pdf,image/*"/>
          <DocUpload label="Гэрээ"                       fieldKey="doc_contract"           preview={previews.doc_contract}          onFile={onFile} editing={editing} accept=".pdf,image/*"/>
          <DocUpload label="Танилцуулга"                 fieldKey="doc_company_intro"      preview={previews.doc_company_intro}     onFile={onFile} editing={editing} accept=".pdf,.doc,.docx,image/*"/>
        </div>
      </Section>

      {/* 7. Банкны мэдээлэл */}
      <Section icon={CreditCard} title="Банкны мэдээлэл">
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14 }}>
          <FInput label="Банкны нэр"    value={form.bank_name}           editing={editing} onChange={(v:string)=>F("bank_name",v)}/>
          <FInput label="Дансны дугаар" value={form.bank_account_number} editing={editing} onChange={(v:string)=>F("bank_account_number",v)} mono/>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:14 }}>
          <FInput  label="НӨАТ дугаар" value={form.vat_number}  editing={editing} onChange={(v:string)=>F("vat_number",v)}  mono/>
          <FInput  label="SWIFT код"   value={form.swift_code}  editing={editing} onChange={(v:string)=>F("swift_code",v)}  mono/>
          <FInput  label="IBAN"        value={form.iban}        editing={editing} onChange={(v:string)=>F("iban",v)}        mono/>
          <FSelect label="Валют"       value={form.currency}    editing={editing} onChange={(v:string)=>F("currency",v)}
