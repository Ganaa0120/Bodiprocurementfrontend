...
import {
  Loader2, Building2, MapPin, Briefcase, FileText,
  CreditCard, X, Check, Save, Plus, ChevronDown,
} from "lucide-react";
...
export default function CompanyProfilePage() {
  ...
  const [dirs,             setDirs]             = useState<{ id:number; label:string; children:{ id:number; label:string }[] }[]>([]);
  ...
  function AddDirectionRow({ dirs, selDirs, onAdd }: { dirs:DirItem[]; selDirs:SelDir[]; onAdd:(mainId:number)=>void }) {
  ...
  const handleSave = async () => {
    ...
    const errs = validateMongolianForm(form, ["sum_duureg",
          ...
          <div style={{ display:"flex", gap:8, justifyContent: isMobile ? "flex-end" : "flex-start" }}>
            ...
            <button onClick={handleSave} disabled={saving}
              style={{ display:"flex", alignItems:"center", gap:5,
                ...
                fontFamily:"inherit", flex: isMobile ? 1 : "none" as any,
                justifyContent:"center" }}>
              {saving ? <><Loader2 size={13} style={{ animation:"spin .8s linear infinite" }}/> Хадгалж байна...</>
      ...
      <Section icon={Building2} title="БАЙГУУЛЛАГЫН ҮНДСЭН МЭДЭЭЛЭЛ">
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:14, marginBottom:14 }}>
          <FInput label="Байгууллагын нэр *" value={form.company_name} editing={editing}
            onChange={(v:string) => { F("company_name",v); setFieldErrors(p=>({...p, company_name:v&&!isMongolian(v)?"Монгол үсгээр бичнэ үү":""})); }}
      ...
      <Section icon={Briefcase} title="ҮЙЛ АЖИЛЛАГААНЫ МЭДЭЭЛЭЛ">
        <div style={{ marginBottom:16 }}>
          ...
          {editing ? (
                ...
                return (
                  <div key={sel.main_id} style={{ borderRadius:12, border:"1.5px solid #6366f1",
                    ...
                    {main.children?.length>0 && (
                      <div style={{ padding: isMobile?"8px 10px 10px 10px":"10px 14px 12px 44px",
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {selDirs.length===0 ? <div style={{ fontSize:13, color:"#cbd5e1" }}>—</div> : (
                selDirs.map(sel => {
                  const main = dirs.find(d => Number(d.id)===Number(sel.main_id));
                  if (!main) return null;
                  return (
                    <div key={sel.main_id} style={{ display:"flex", alignItems:"flex-start",
                      flexWrap:"wrap", gap:8 }}>
                      <span style={{ fontSize:12, fontWeight:600, color:"#4f46e5", background:"#eef2ff",
                        border:"1px solid #c7d2fe", padding:"3px 10px", borderRadius:99,
                        whiteSpace:"nowrap" as const }}>{main.label}</span>
                      {sel.sub_ids.length>0 && (
                        <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
                          {sel.sub_ids.map(sid => {
                            const sub = main.children?.find((c:any) => Number(c.id)===Number(sid));
                            return sub ? (
                              <span key={sid} style={{ fontSize:11, color:"#64748b", background:"#f8fafc",
                                border:"1px solid #e2e8f0", padding:"3px 8px", borderRadius:99 }}>
                                {sub.label}
                              </span>
                            ) : null;
                          })}
      ...
      {editing && (
        <div style={{ display:"flex", justifyContent:"flex-end", gap:10,
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? <><Loader2 size={14} style={{ animation:"spin .8s linear infinite" }}/> Хадгалж байна...</>
