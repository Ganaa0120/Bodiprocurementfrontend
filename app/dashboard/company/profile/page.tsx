...
export default function CompanyProfilePage() {
  ...
  // ✅ Extra files: шинэ File[] + хадгалагдсан URL[]
      ...
      <div>
        <button type="button" onClick={()=>setOpen(p=>!p)}
          style={{ width:"100%", display:"flex", alignItems:"center", gap:8, padding:"10px 14px",
            borderRadius:12, border:"1.5px dashed #c7d2fe", background:"#eef2ff",
          ...
          <div style={{ marginTop:6, background:"white", borderRadius:12,
            ...
            {available.map(d => (
              <button key={d.id} type="button" onClick={()=>{ onAdd(d.id); setOpen(false); }}
                ...
                <div style={{ width:28, height:28, borderRadius:8, flexShrink:0, background:"#eef2ff",
                  display:"flex", alignItems:"center", justifyContent:"center",
                  fontSize:12, fontWeight:700, color:"#6366f1" }}>{d.label[0]}</div>
                <div>
  ...
  const handleSave = async () => {
    ...
    if(form.register_number && form.register_number.length!==7){
      ...
      setError("Регистрийн дугаар 7 оронтой байх ёстой"); return;
    }
    ...
    directors.forEach((d:any,idx:number)=>{
      if(d.position  &&!/^[\u0400-\u04FF\s\-]+$/.test(d.position))   dirErrors[`${idx}_position`] ="Крилл үсгээр бичнэ үү";
      if(d.last_name &&!/^[\u0400-\u04FF\s\-]+$/.test(d.last_name))  dirErrors[`${idx}_last_name`]="Крилл үсгээр бичнэ үү";
      if(d.first_name&&!/^[\u0400-\u04FF\s\-]+$/.test(d.first_name)) dirErrors[`${idx}_first_name`]="Крилл үсгээр бичнэ үү";
      if(d.phone&&(!/^\d+$/.test(d.phone)||d.phone.length!==8))       dirErrors[`${idx}_phone`]="8 оронтой тоо оруулна уу";
      if(d.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d.email))        dirErrors[`${idx}_email`]="И-мэйл хаяг буруу байна";
    });
    if(Object.keys(dirErrors).length>0){
      setDirectorFieldErrors(dirErrors);
      setError("Гүйцэтгэх захирлын мэдээлэл буруу байна"); return;
    }
    ...
    // ✅ Extra files backend руу явуулна
    ...
    try {
      ...
      // ✅ Extra docs шинэчилнэ
      const newUrls = Array.isArray(updated.extra_documents) ? updated.extra_documents : extraFileUrls;
    ...
    <div style={{ maxWidth:1200, margin:"0 auto", display:"flex", flexDirection:"column",
        ...
        <div style={{ display:"grid", gridTemplateColumns:g2, gap:14, marginBottom:14 }}>
          <FInput label="Регистрийн дугаар *" value={form.register_number} editing={editing}
            onChange={(v:string)=>{
              ...
              setFieldErrors(p=>({...p,register_number:digits&&digits.length<7?"7 оронтой тоо оруулна уу":""}));
            }}
            fieldError={fieldErrors.register_number} mono/>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {selDirs.length===0
                ...
                : selDirs.map(sel=>{
                    ...
                    if(!main) return null;
                    return (
                      <div key={sel.main_id} style={{ display:"flex", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
                        ...
                        {sel.sub_ids.length>0&&(
                          <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
                            {sel.sub_ids.map(sid=>{
                              ...
                              return sub?(<span key={sid} style={{ fontSize:11, color:"#64748b",
                                background:"#f8fafc", border:"1px solid #e2e8f0",
                                padding:"3px 8px", borderRadius:99 }}>{sub.label}</span>):null;
                            })}
                          </div>
                        )}
                      </div>
      ...
      <Section icon={FileText} title="КОМПАНИЙН БАРИМТ БИЧИГ">
        ...
        <div style={{ borderTop:"1px solid #f1f5f9", paddingTop:14 }}>
          ...
          {/* Хадгалагдсан URL файлууд — ногоон */}
          {extraFileUrls.length>0&&(
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:10 }}>
              {extraFileUrls.map((url,i)=>{
                const name = decodeURIComponent(url.split("/").pop()?.split("?")[0]||`Файл ${i+1}`);
                return (
                  <div key={`url-${i}`} style={{ display:"flex", alignItems:"center", gap:10,
                    padding:"8px 12px", borderRadius:10, background:"#f0fdf4", border:"1px solid #a7f3d0" }}>
                    <FileText size={14} style={{ color:"#059669", flexShrink:0 }}/>
                    <a href={url} target="_blank" rel="noreferrer"
                      style={{ fontSize:12, color:"#059669", flex:1, overflow:"hidden",
                        textOverflow:"ellipsis", whiteSpace:"nowrap" as const,
                        textDecoration:"none", fontWeight:500 }}>
                      {name}
                    </a>
                    <span style={{ fontSize:10, color:"#6ee7b7", fontWeight:600,
                      whiteSpace:"nowrap" as const }}>✓ Хадгалагдсан</span>
                    {editing&&(
                      <button type="button" onClick={()=>setExtraFileUrls(p=>p.filter((_,j)=>j!==i))}
                        style={{ background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)",
                          borderRadius:6, padding:"3px 8px", cursor:"pointer", fontSize:11,
                          color:"#ef4444", fontFamily:"inherit", whiteSpace:"nowrap" as const }}>
                        Хасах
                      </button>
                    )}
                  </div>
          ...
          {/* Шинэ нэмсэн файлууд — шар */}
          {extraFiles.length>0&&(
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:10 }}>
              {extraFiles.map((f,i)=>(
                <div key={`new-${i}`} style={{ display:"flex", alignItems:"center", gap:10,
      ...
      {showSuccessModal&&(
        <div style={{ position:"fixed", inset:0, zIndex:200, background:"rgba(0,0,0,0.45)",
          ...
          <div style={{ width:"100%", maxWidth:isMobile?"100%":420, background:"white",
            ...
            <h2 style={{ fontSize:20, fontWeight:700, color:"#0f172a", margin:"0 0 10px" }}>Бүртгэл амжилттай!</h2>
                ...
                <div key={i} style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                    <div style={{ width:28, height:28, borderRadius:"50%",
                      ...
                      display:"flex", alignItems:"center", justifyContent:"center",
                      fontSize:12, color:"white", fontWeight:700 }}>{i===0?"✓":i+1}</div>
                    <span style={{ fontSize:10, color:i===0?"#10b981":i===1?"#f59e0b":"#94a3b8", fontWeight:500 }}>{step}</span>
                  </div>
