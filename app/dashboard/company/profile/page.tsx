...
export default function CompanyProfilePage() {
  ...
  function AddDirectionRow({
    ...
    const available = dirs.filter(
      (d) => !selDirs.find((s) => s.main_id === d.id),
    );
      ...
      .then((d) => {
        if (d.success && (d.organization || d.user)) {
          ...
          const lf = u.final_beneficial_owners?.length
            ...
            : [{ ...BLANK_FINAL }];
          const ld = u.executive_directors?.length
            ? u.executive_directors
            : [{ ...BLANK_EXEC }];
          const lp = u.special_permissions?.length
            ? u.special_permissions
            : [{ ...BLANK_PERM }];
          setOwners(lo);
          setOwnerSnap(lo);
          setFinalOwners(lf);
  ...
  const toggleMain = (mainId: number) => {
    setSelDirs((p) => {
      ...
      return exists
        ? p.filter((d) => d.main_id !== mainId)
        : [...p, { main_id: mainId, sub_ids: [] }];
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const lf = updated.final_beneficial_owners?.length
        ...
        : [{ ...BLANK_FINAL }];
      const ld = updated.executive_directors?.length
        ? updated.executive_directors
        : [{ ...BLANK_EXEC }];
      const lp = updated.special_permissions?.length
        ? updated.special_permissions
        : [{ ...BLANK_PERM }];
      setOwners(lo);
      setOwnerSnap(lo);
      setFinalOwners(lf);
    ...
    >
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {editing ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {dirs.filter((d) => !selDirs.find((s) => s.main_id === d.id))
                .length > 0 && (
                <AddDirectionRow
                  dirs={dirs}
                  selDirs={selDirs}
                ...
                <FSelect
                  ...
                  onChange={(v: string) => {
                    ...
                    setSpecPerms((p) =>
                      p.map((x, i) =>
                        i === idx
                          ? { ...x, type_label: v, type_id: found?.id || null }
                          : x,
                      ),
                    );
                  }}
                  options={permTypes.map((t) => t.label)}
      ...
      <Section icon={FileText} title="КОМПАНИЙН БАРИМТ БИЧИГ">
        ...
        <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: 14 }}>
          ...
          {/* View mode — файл байхгүй бол */}
          {!editing && extraFiles.length === 0 && (
            <div style={{ fontSize: 13, color: "#cbd5e1" }}>—</div>
          )}
        </div>
      </Section>
